...
**用户界面**
- ✅ **Windows Fluent Design System**：现代 UI 设计风格，与 Windows 11 原生应用视觉一致
- ✅ **深色/浅色主题**：支持自动跟随系统主题或手动切换
- ✅ **圆角设计**：按钮、输入框、对话框等组件采用现代圆角风格
- ✅ **流畅动画**：按钮悬停/按下动画，主题切换平滑过渡
- ✅ **中英文支持**：完整的中文界面和菜单，无字体乱码
- ✅ **DPI 自适应**：完美适配 125%、150%、200% 等高分辨率

**部署选项**
- ✅ **跨平台打包**：支持 JAR、app-image、EXE 等多种格式
- ✅ **便携式 EXE**：无需安装 Java，开箱即用
...
- **Swing**：原生 GUI 界面，支持完整中文和高分辨率
- **FlatLaf 3.6**：Windows Fluent Design 主题库，提供深/浅色模式自动切换
- **GridBagLayout**：物理键盘布局渲染
- **JNativeHook 2.2.2**：全局键盘钩子监听库
- **JNA 5.14.0 + jna-platform**：Windows API 调用（深色模式检测）
- **Gson 2.10.1**：JSON 配置管理（含主题持久化）
